...
try {
    $sError = ""
    $oApplicationEvents = Get-EventLog -ComputerName $sComputer -LogName "Application" -EntryType Critical,Error,Warning -after (Get-Date).AddHours($iApplicationEventLastHours * -1) -ErrorAction Stop
} catch [Exception]{
    $oError = $_
