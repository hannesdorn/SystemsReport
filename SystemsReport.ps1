# Systems Report
# powershell.exe SystemsReport.ps1 host smtp from to
# powershell -File SystemsReport.ps1 smtp hannesd@private.dorn.local hannesd@private.dorn.local
...
# Set free disk space threshold below in percent
$thresholdspace = 20
[int]$EventNum = 50
...
<head>
    ...
    <!--
        ...
        div.first
        {
            padding-right: 20px; border-right: 1px  grey solid;
        }
        div.second
        {
            margin-left: 30px;
...
foreach($computer in $computers)
{
	...
	Create-PieChart -FileName ((Get-Location).Path + "\" + $CurrentTime + " RAM $computer") $FreeRAM, $UsedRAM
	$attachments += $CurrentTime + " RAM $computer.png"
...
@"
	...
	<img src="cid:$CurrentTime RAM $computer.png" alt="$computer RAM">
...
$HTMLmessage | Out-File ((Get-Location).Path + "\" + $CurrentTime + " Report.html")

# Email our report out
#send-mailmessage -from $fromemail -to $users -subject "Systems Report" -Attachments $attachments -BodyAsHTML -body $HTMLmessage -priority Normal -smtpServer $server
...
$msg.subject = "Systems Report"
foreach($computer in $computers) {
    $msg.subject += " $computer"
}
$msg.IsBodyHtml = $true
